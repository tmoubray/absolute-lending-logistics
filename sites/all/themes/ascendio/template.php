...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for ascendio
 */

// Auto-rebuild the theme registry during theme development.
if ( theme_get_setting( 'ascendio_clear_registry' ) ) {
	// Rebuild .info data.
 ...
 */
function ascendio_html_head_alter( &$head_elements ) {
 ...
 */
function ascendio_preprocess_search_block_form( &$vars ) {
 ...
 */
function ascendio_preprocess( &$vars, $hook ) {
	$vars['ascendio_path'] = base_path() . path_to_theme();
 ...
 */
function ascendio_preprocess_html( &$vars ) {
	$vars['doctype'] = _ascendio_doctype();
	$vars['rdf'] = _ascendio_rdf( $vars );
	...
	if ( !$vars['is_front'] ) {
		...
		if ( isset( $page_name ) ) {
			$vars['classes_array'][] = ascendio_id_safe( 'page-' . $page_name );
		}

		$vars['classes_array'][] = ascendio_id_safe( 'section-' . $section );
	...
	if ( ( $d['destination'] == 'blog' ) && ( theme_get_setting( 'ascendio_blog_title' ) != "" ) ) {
		$vars['title'] = theme_get_setting( 'ascendio_blog_title' );
		$vars['head_title'] = theme_get_setting( 'ascendio_blog_title' );
	}
}
 ...
 */
function ascendio_preprocess_page( &$vars ) {
 ...
 */
function ascendio_preprocess_maintenance_page( &$vars ) {
	...
	drupal_add_css( drupal_get_path( 'theme', 'ascendio' ) . '/css/maintenance-page.css' );
}
 ...
 */
function ascendio_preprocess_node( &$vars ) {
	// Striping class
 ...
 */
function ascendio_preprocess_block( &$vars, $hook ) {
 ...
 */
function ascendio_menu_tree( $vars ) {
 ...
 */
function ascendio_field__taxonomy_term_reference( $vars ) {
	$output = '';
 ...
 */
function ascendio_breadcrumb( $vars ) {
	$breadcrumb = isset( $vars['breadcrumb'] ) ? $vars['breadcrumb'] : array();

	$condition = theme_get_setting( 'ascendio_breadcrumb_hideonlyfront' ) ? count( $breadcrumb ) > 1
			: !empty( $breadcrumb );
	$separator = theme_get_setting( 'ascendio_breadcrumb_separator' );

	if ( theme_get_setting( 'ascendio_breadcrumb_showtitle' ) ) {
 ...
 * @return bool
 */
function ascendio_tabs_float() {
	$float = (bool) theme_get_setting( 'ascendio_tabs_float' );
	$float_node = (bool) theme_get_setting( 'ascendio_tabs_node' );
...
function ascendio_id_safe( $string ) {
	// Strip accents
 ...
 * @ingroup themeable
 */
function ascendio_menu_link( array $vars ) {
	...
	$element['#attributes']['class'][] = ascendio_id_safe( $element['#title'] );
 ...
 */
function ascendio_preprocess_menu_local_task( &$vars ) {
 ...
 */
function ascendio_menu_local_tasks( &$vars ) {
	$output = '';
 ...
 */
function ascendio_preprocess_comment( &$variables ) {
...
/* Region options */
function ascendio_region_preffix ( $reg ) {
	$block_bg_type = theme_get_setting( 'ascendio_' . $reg . '_bg_type' );
	$block_bg_img = theme_get_setting( 'ascendio_' . $reg . '_bg_img' );
	$block_bg_parallax = theme_get_setting( 'ascendio_' . $reg . '_bg_parallax' );
	$bg_video = theme_get_setting( 'ascendio_' . $reg . '_bg_video' );
	$bg_video_start = theme_get_setting( 'ascendio_' . $reg . '_bg_video_start' );
	$fullwidth = theme_get_setting( 'ascendio_' . $reg . '_fullwidth' );
	$invert = theme_get_setting( 'ascendio_' . $reg . '_invert' );
	...
	$output_preffix = '<div id="' . $reg . '_wrapper"' . $classes . $styles . $attributes . '>' . $content;
		if ( !$fullwidth ) {
			$output_preffix .= '<div class="container">';
				$output_preffix .= '<div class="row">';
					$output_preffix .= '<div class="col-sm-12">';
		}

	print $output_preffix;
}
function ascendio_region_suffix ( $reg ) {
	$fullwidth = theme_get_setting( 'ascendio_' . $reg . '_fullwidth' );
	$block_bg_parallax = theme_get_setting( 'ascendio_' . $reg . '_bg_parallax' );
	$block_bg_img = theme_get_setting( 'ascendio_' . $reg . '_bg_img' );
...
function ascendio_form_contact_site_form_alter( &$form, &$form_state, $form_id ) {
